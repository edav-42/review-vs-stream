...
# TODO replace preset page numbers
...
str(df)
# del useless columns
df <- subset(df, select = -c(V6, V8, V9))
# rename columns
df <- df %>%
  rename(
    "Metacritic_Score" = "V1",
...
# add uid
to_lower_alnum <- function(s) {
  tolower(gsub("[^[:alnum:]]", "", s))
}
strip_album_name <- function(s) {
  paste(substr(s,1,3), substr(s, nchar(s)-2, nchar(s)), sep = "")
}
df <- df %>%
  mutate(id = paste(
    to_lower_alnum(Singer),
    Release_Year,
    strip_album_name(to_lower_alnum(Album_Name)),
    sep = ""
  ))
...
```{r}
# export to csv
write.csv(df, "data/metacritic_data_new.csv", row.names = FALSE)
```
