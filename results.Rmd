...
```
```{r}
fig <- plot_ly(df, x = ~X.Ratings, y = ~RYM.Rating, text = ~ Album_Name, color = ~Metacritic_Score, type = 'scatter', mode = 'markers', colors = 'YlOrBr',
        sizes = c(10, 50),
...
## Metacritic VS Rate Your music
```{r}
ggplot(df, aes(Metacritic_Score, RYM.Rating)) + 
  geom_point(size = 0.03, alpha = 0.5) + 
...
ggplot(df, aes(Metacritic_Score, RYM.Rating, colour = Fct_Release_Year)) + 
  geom_point(size = 0.03, alpha = 0.5) + 
  geom_smooth() +
